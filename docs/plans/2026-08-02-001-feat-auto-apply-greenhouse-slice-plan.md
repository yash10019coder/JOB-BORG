...
- **Exception Swallowing in draft_auto_apply Silently Drops Jobs with Zero User Visibility** — U6 (Drafting orchestration and draft_auto_apply task) (P1, feasibility, confidence 100)

  In draft_auto_apply(user_id, job_id), unexpected exceptions raised by draft_for (e.g., Anthropic API network errors, database timeouts, or unexpected schema formatting faults) are caught by a blanket except Exception: block that logs the error and returns None without persisting an AutoApplyDraft record. When a user clicks 'Auto-apply', the UI redirects immediately with a 'drafting...' flash message directing them to the review queue. Because no DRAFTED, EXCLUDED, or FAILED row is created, the job silently vanishes from the review queue. Furthermore, returning None makes Celery treat the task as SUCCESS, preventing task retries and masking failure rates in observability tools.

- **Module Import-Time Freezing and Formula Flaw in Celery Submission Task Time Limits** — U7 (Send flow, submit_auto_apply_draft task, and staleness sweep) (P1, feasibility, confidence 100)

  In apps/auto_apply/tasks.py, _SUBMIT_SOFT_TIME_LIMIT_SECONDS is calculated as max(30, settings.AUTO_APPLY_SENDING_TIMEOUT_SECONDS - 60) and _SUBMIT_TIME_LIMIT_SECONDS adds 30 to that soft limit. Because these variables are evaluated at module import time, dynamic settings overrides (such as in tests via @override_settings or runtime environment changes) fail to update Celery's time limits. Furthermore, if AUTO_APPLY_SENDING_TIMEOUT_SECONDS is set to 60 or less (common in dev/test environments), _SUBMIT_TIME_LIMIT_SECONDS evaluates to 60, equal to or exceeding the sweep timeout. This violates the explicitly stated architectural invariant that Celery must kill long-running tasks before sweep_stale_auto_apply_drafts marks in-flight drafts as FAILED out from under them, leading to race conditions and split-brain state between JobApplication and AutoApplyDraft.

- **Empty CAPTCHA Solver Abstraction (U5) Adds Framework Bloat with Zero Current Implementations** — U5 (Pluggable CAPTCHA-solving client) / U3 (Greenhouse client) (P1, scope-guardian, confidence 100)

  The plan creates a dedicated package (apps/auto_apply/captcha/), protocol, provider registry, environment variables, and unit tests for an interface that has zero production implementations in this slice. Since runtime behavior simply falls back to failing closed whenever a CAPTCHA challenge is detected, shipping a multi-provider registry and empty interface layer adds implementation complexity and file bloat without earning its keep. Furthermore, U5 states the solver is invoked during inspect(), whereas U3's technical design pseudocode for inspect() immediately raises GreenhouseFormChallenged without calling solve().

- **U2 Omits reason_code and form_schema_snapshot Required by Monitoring and Drift Detection** — U2 (Core data models) / Monitoring (P1, coherence, confidence 100)

  The operational observability specifications explicitly mandate grouping submission failures by reason_code, while automated schema-drift defense relies on comparing inspected form structures against a stored form_schema_snapshot in AutoApplyDraft. However, unit U2's model specification and migration field inventory completely omit both fields. When implementers build U2 as written, integration with U6 (drafting) and U7 (submission) will trigger runtime database field errors, disabling telemetry ingestion and schema-drift protection.

- **Indefinite Retention of Unindexed PII in Debug Screenshots and Logs Without TTL** — U3 (Greenhouse client) / U7 (Send flow) (P1, security-lens, confidence 100)

  When automated form inspection or application submission fails, unit U3 generates browser screenshots and exports raw accessibility DOM trees to worker storage for diagnosis. Because application forms render sensitive user PII—including names, email addresses, phone numbers, work history, and gender or race self-identification questions—these dump files contain unencrypted personal data. The plan specifies no retention windows, TTLs, or cleanup tasks for these files, causing unbounded accumulation of untyped user PII across worker filesystems and leaving data exposed even after user profile deletion.

- **Missing Rate Limiting and Concurrency Caps on Expensive Playwright and LLM Endpoints** — U8 (Web UI trigger and review queue) / U6 / U7 (P1, security-lens, confidence 100)

  Unit U8 specifies interactive HTTP POST endpoints that initiate heavyweight asynchronous Celery tasks: complex LLM reasoning in draft_auto_apply (U6) and memory-intensive headless browser execution via Playwright in submit_auto_apply_draft (U7). The specification omits rate limiting, request throttling, or active per-user concurrency caps on these triggers. Because task dispatch is unbounded, an authenticated user—or an automated script exploiting a compromised session—can repeatedly invoke auto-apply endpoints, exhausting worker RAM with concurrent Chromium instances and driving massive financial DoS through uncontrolled third-party Anthropic API charges.

- **Orphaned Resume File Retention in Storage on Account Deletion or File Replacement** — U1 (Profile resume & target locations fields) / Security Architecture (P1, security-lens, confidence 100)

  Unit U1 extends Profile with a FileField for document uploads (resume) alongside a parsed text column (resume_text). Under standard Django storage mechanics, when a user deletes their account or uploads a revised resume document over an existing file, the unreferenced file is NOT automatically purged from the underlying filesystem or S3 storage bucket. Furthermore, the plan leaves account cascade-deletion as an open question under 'Resolve Before Implementation' instead of specifying a concrete architecture. Leaving abandoned resumes behind in object storage violates basic data minimization principles and creates major compliance exposure under right-to-be-forgotten regulations (GDPR/CCPA).

- **Undefined Access Control and Public Endpoint Exposure Risk for Resume Files** — U1 (Profile resume & target locations fields) / Security Architecture (P1, security-lens, confidence 100)

  Unit U1 provisions a new resume file upload field on Profile without defining access control boundaries, signed URL routing, or bucket privacy policies for the saved files. Under standard Django configurations, media URLs default to public serving without user authorization checks, enabling anyone with a generated link or enumerated S3 object key to download arbitrary applicant resumes containing unencrypted personal identity data (names, home addresses, phone numbers, employment timelines). Failing to restrict object access at the architectural layer introduces severe data disclosure vulnerabilities.

- **Re-Trigger Guard on Already-Applied Jobs Omitted from U6 and U8** — U8 (Web UI trigger and review queue) / U6 (Drafting orchestration) (P1, coherence, confidence 100)

  Under top-level architecture constraints and prior bug learnings, the document emphasizes that an idempotent re-trigger guard is required to prevent drafting or submitting duplicate auto-applications for already-applied jobs. However, unit U6 (draft_auto_apply) and unit U8 (Web UI trigger view) omit pre-check queries against existing JobApplication or AutoApplyDraft records. Because implementers build directly against unit specifications, the resulting code will permit users to repeatedly re-trigger drafting and duplicate submissions on previously completed applications, driving redundant LLM inference costs and generating applicant submission spam at employer ATS portals.

- **Pluggable LLM Provider Registry is Premature Over-Abstraction for a Single Vendor** — U4 (Pluggable LLM client and Anthropic implementation) (P2, scope-guardian, confidence 100)

  Unit U4 specifies building an abstract provider protocol class, dynamic provider dictionary factories (PROVIDER_REGISTRY), and runtime resolution through settings (AUTO_APPLY_LLM_PROVIDER) for what is explicitly defined as a single-vendor slice exclusively utilizing Anthropic Claude. Constructing multi-provider factory architectures before a second consumer actually exists violates YAGNI and right-sized scoping principles, burdening the implementation with needless indirection, testing boilerplate, and directory complexity.

- **Unsanitized PII Transfer to Third-Party LLM Service Without Explicit User Consent** — U4 (Pluggable LLM client) / Security Scope (P2, security-lens, confidence 100)

  When unit U6 compiles prompt contexts for answer resolution via unit U4 (AnthropicClient), it transmits the applicant's complete profile attributes, custom explicit answers, and raw parsed resume contents (resume_text) over the internet to external Anthropic cloud endpoints. Resumes inherently carry detailed personal identity records, including legal names, residential street addresses, personal cell phone numbers, employment timelines, and educational histories. The design neither specifies a data sanitization or pre-anonymization pass prior to dispatching external API requests, nor mandates displaying explicit data transmission disclosures and opt-in consent warnings on the auto-apply UI triggers. Exporting untyped personal identification data to third-party generative AI processors without transparent user disclosure introduces significant regulatory exposure under modern data privacy statutory frameworks.
